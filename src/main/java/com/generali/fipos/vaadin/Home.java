...
public class Home extends VerticalLayout {
	public Home() {
		...
		add(new Paragraph("When a hierarchical TreeGrid is expanded and it is necessary to scroll to an item, that doesn't work, because of the lazy loading. HOW to reproduce: clicking on the Button should scoll to the item <TEST>. But it doesn't. when manually scolling from the end to the beginning and clicking on the button - it will scroll correctly (maybe after a view tries). In the console the NodeIndex of the node is shown, but it doesn't correspond to the rowindex (NodeIndex-1) within the DOM!"));
	} 
}
